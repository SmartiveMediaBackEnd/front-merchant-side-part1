...
import {
	AddProductSchemaSchemaValues,
	defaultProductValues,
	ProductSchema,
} from '../../pages/ProductsPage/tabs/_comp/NewProduct/Pages/Configurable/utils';
...
export const FormStoreProvider = ({ children }: { children: React.ReactNode }) => {
	...
	const submitHandler = async (values: AddProductSchemaSchemaValues) => {
		...
		const transformedVariants = formData.variants?.reduce(
			(acc: Record<string, any>, variant: any, index: number) => {
				if (!variant || !variant.sku) {
					console.warn(`Variant at index ${index} is invalid`, variant);
					return acc;
				}
				const variantKey = `variant_${index}`;
				const inventoriesObject =
					variant.inventories?.reduce((obj: InventoriesObject, item: InventoryInterface) => {
		...
		const specificationsPayload = formData.specifications.reduce(
			(acc: Record<string, any>, specifications: any, index: number) => {
				if (
					!specifications.name_ar ||
					!specifications.value_ar ||
					!specifications.name_en ||
					!specifications.value_en
				) {
		...
		const faqsPayload = formData.faqs.reduce(
			(acc: Record<string, any>, faq: any, index: number) => {
		...
		try {
			if (id) {
				...
				if (response?.payload?.code === 200) {
					...
					const specificationsResponse = await dispatch(
						postSpecifications({ specs: send_data, product_id: response.payload.data.id }),
					);
					...
					const faqsResponse = await dispatch(
						postFaqs({ faqs: send_data_faqs, product_id: response.payload.data.id }),
					);
			...
			if (variant.sku) {
				...
				if (variant.inventories) {
					variant.inventories.forEach((inventory: InventoryInterface, inventoryIndex: number) => {
						...
						formStore.setValue(
							`variants.${index}.inventories.${inventoryIndex}.quantity`,
							inventory.quantity,
						);
					});
				}
				...
				if (variant.attributeValues) {
					variant.attributeValues.forEach(
						(attributeValue: AttributeValue, attributeIndex: number) => {
							formStore.setValue(
								`variants.${index}.attributeValues.${attributeIndex}.id`,
								attributeValue.id,
							);
							formStore.setValue(
								`variants.${index}.attributeValues.${attributeIndex}.name`,
								attributeValue.name,
							);
						},
					);
